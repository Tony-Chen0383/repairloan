/* RepairLoan local QR renderer.
   QR core: QRCode for JavaScript by Kazuhiko Arase (MIT).
   Bundled locally so GitHub Pages does not depend on a third-party CDN.
*/
(function(global){
  "use strict";
  const factories = {
...
module.exports = QRCode;
  ...
  function localRequire(id){
    ...
    if(!f) throw new Error("QR module not found: "+id);
    const module = {exports:{}};
    cache[id] = module;
    f(module,module.exports,localRequire);
  ...
  function toCanvas(canvas,text,opts){
    ...
    return new Promise((resolve,reject)=>{
      try{
        const levelName = String(opts.errorCorrectionLevel || "M").toUpperCase();
        const level = QRErrorCorrectLevel[levelName] || QRErrorCorrectLevel.M;
        const qr = new QRCodeCore(-1, level);
        qr.addData(String(text || ""));
        qr.make();

        const count = qr.getModuleCount();
        const margin = Number.isFinite(opts.margin) ? Math.max(0, Math.floor(opts.margin)) : 4;
        const cssWidth = Math.max(96, Math.floor(opts.width || 220));
        const dpr = Math.max(1, global.devicePixelRatio || 1);
        const desiredPx = Math.floor(cssWidth * dpr);
        const cells = count + margin * 2;
        const cell = Math.max(1, Math.floor(desiredPx / cells));
        const px = cell * cells;
  ...
  function toSvg(text,opts){
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" role="img" aria-label="QR CODE" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#fff"/><path d="${d}" fill="#000"/></svg>`;
  }

  global.RepairLoanQR = {toCanvas,toSvg};
})(window);


/* === RepairLoan main app V2.0.4.3 === */
(() => {
