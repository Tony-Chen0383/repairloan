...
function setupSignature(){
  ...
  const cssW=Math.max(280,Math.round(r.width||c.parentElement?.clientWidth||600));
  const cssH=220;
  ...
  if('PointerEvent' in window){
    ...
    const touchPoint=e=>{
      ...
      return t?{clientX:t.clientX,clientY:t.clientY,preventDefault:()=>e.preventDefault()}:e;
    };
    c.addEventListener('touchstart',e=>start(touchPoint(e)),{passive:false});
    c.addEventListener('touchmove',e=>move(touchPoint(e)),{passive:false});
    c.addEventListener('touchend',e=>stop(touchPoint(e)),{passive:false});
  }
}
function clearSignature(){const c=$('sig');if(ctx){ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,c.width,c.height);ctx.restore()}signatureDrawn=false}
function sigData(){return $('sig').toDataURL('image/png')}
