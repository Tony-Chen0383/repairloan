...
function updateSignatureOrientationGuard(){
  ...
  if(guide){
    if(manual){
      ...
      guide.innerHTML='<b>自動旋轉橫向簽名</b><span>請將手機轉成橫向，並讓手機頂端／前鏡頭位於左側。</span>';
    }
  }
  return st.allowed;
}
...
function scheduleAutoSignatureOpen(){
  ...
  const tick=()=>{
    ...
    if(!autoLandscapeState().allowed)return;
...
function maybeAutoSignatureOpen(){
  ...
  if(autoLandscapeState().allowed&&sigState.inView&&readyForAutoSignature()&&!sigState.suppressAutoOpen)scheduleAutoSignatureOpen();
}
function setupSignature(){
  ...
  // 自動模式：只有簽名區進入視窗、前置資料完成、手機實體轉到指定橫向才自動開啟。
  ...
  const sync=()=>{
    ...
    if(wrap.classList.contains('fullscreen')){
      ...
      if(!autoLandscapeState().allowed&&sigState.suppressAutoOpen)sigState.suppressAutoOpen=false;
