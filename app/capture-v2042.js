(() => {
const cfg=window.REPAIRLOAN_CONFIG,API=window.RepairLoanAPI,R=window.RepairLoanRenderer;const token=new URLSearchParams(location.search).get('token')||'';const sides=[['front','正面'],['back','背面'],['left','左側'],['right','右側'],['top','上方'],['bottom','下方']];let loan=null,customerIdNo='',photos={},idPhotos={},signatureDrawn=false,ctx=null,drawing=false,last=null,busy=false;let sigState={hasInk:false,normalizedCanvas:null,fullscreen:false,suppressAutoOpen:false,autoOpenTimer:null,inView:false,autoArmed:false,exiting:false,returnScrollY:0};
...
async function init(){
  ...
  try{
    ...
    loan=r.loan;
    customerIdNo=String(r.customer_id_no||'').trim();
    if(!customerIdNo)throw new Error('此借用單缺少送單前證件號碼，請由門市重新建立借用單');
    $('idNoDisplay').value=maskId(customerIdNo)+'｜已由門市送單前帶入';
...
function isProbablyMobileSignatureDevice(){
  return (navigator.maxTouchPoints||0)>0 || window.matchMedia?.('(pointer:coarse)')?.matches || /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent||'');
}
function autoLandscapeState(){
  const landscape=window.innerWidth>window.innerHeight;
  if(!landscape)return {allowed:false,reason:'portrait',angle:null};

  // 桌機僅供開發測試；手機／平板才做實體方向限制。
  if(!isProbablyMobileSignatureDevice())return {allowed:true,reason:'desktop',angle:null};
  ...
  // 固定規格：前鏡頭／手機頂端在左側 = landscape-secondary / 270°。
  if(type==='landscape-secondary'||angle===270)return {allowed:true,reason:'camera-left',angle};
  if(type==='landscape-primary'||angle===90)return {allowed:false,reason:'camera-right',angle};

  // 部分 Safari 不可靠地回報 primary/secondary；至少仍要求實體橫向。
  if(landscape&&(angle===null||angle===0))return {allowed:true,reason:'safari-landscape-fallback',angle};
  return {allowed:false,reason:'wrong-direction',angle};
}
function currentSignatureAllowed(){
  const wrap=$('signaturePadWrap');
  if(!wrap?.classList.contains('fullscreen'))return false;
  if(wrap.classList.contains('manual-entry'))return true;
  return autoLandscapeState().allowed;
}
function updateSignatureOrientationGuard(){
  const wrap=$('signaturePadWrap');
  if(!wrap||!wrap.classList.contains('fullscreen'))return false;

  const manual=wrap.classList.contains('manual-entry');
  const st=manual?{allowed:true,reason:'manual'}:autoLandscapeState();
  ...
  if(guide){
    if(manual){
      guide.innerHTML='<b>手動橫向簽名</b><span>已直接進入橫向簽名模式，請由左至右簽寫；不需要再開啟自動旋轉。</span>';
    }else if(st.allowed){
      guide.innerHTML='<b>自動旋轉橫向簽名 ✓</b><span>手機頂端／前鏡頭在左側，請由左至右簽寫。</span>';
    }else{
      guide.innerHTML='<b>自動旋轉橫向簽名</b><span>簽名頁已啟動；請讓手機頂端／前鏡頭位於左側，方向正確後即可由左至右簽寫。</span>';
    }
  }
  return st.allowed;
}
...
function signatureStageReady(){
  // 自動旋轉只要求前置拍攝資料完成；「同意條款」仍由最後確認借出 Gate 檢查。
  // 這樣客人可以先簽名，再勾選同意，不會因 checkbox 阻擋自動橫向簽名。
  return sides.every(([k])=>photos[k]?.uploaded) &&
...
function signatureAreaReached(){
  const wrap=$('signaturePadWrap');
  if(!wrap)return false;
  const r=wrap.getBoundingClientRect();
  const vh=window.visualViewport?.height||window.innerHeight||0;
  // 只要客人已滑到簽名區附近，就永久啟用本次頁面的 Auto 模式。
  return r.top < vh*1.15 && r.bottom > -80;
}
function armSignatureAutoIfReached(){
  if(sigState.autoArmed)return true;
  if(signatureAreaReached()){
    sigState.autoArmed=true;
    return true;
  }
  return false;
}
function cancelPendingAutoSignatureOpen(){
  if(sigState.autoOpenTimer){
    clearTimeout(sigState.autoOpenTimer);
    sigState.autoOpenTimer=null;
  }
}
function scheduleAutoSignatureOpen(){
  const wrap=$('signaturePadWrap');
  if(!wrap||wrap.classList.contains('fullscreen')||sigState.suppressAutoOpen||sigState.exiting)return;
  if(!sigState.autoArmed||!signatureStageReady()||!isLandscapeViewport())return;

  cancelPendingAutoSignatureOpen();

  // 跟新機回收同概念：等 Safari / Chrome 橫向 viewport 穩定後，
  // 自動走跟手動相同的滿版簽名入口。
  let lastW=0,lastH=0,stableCount=0,attempt=0;
  const tick=()=>{
    sigState.autoOpenTimer=null;
    if(sigState.suppressAutoOpen||sigState.exiting)return;
    if(!sigState.autoArmed||!signatureStageReady()||!isLandscapeViewport())return;
    ...
    if(stableCount>=2||attempt>=6){
      enterSignatureFullscreen('auto');
      return;
    }
...
function maybeAutoSignatureOpen(){
  ...
  // 重要：只要「已經實體橫向」就先開簽名頁。
  // 若方向不是前鏡頭在左側，進入後由 blocker 明確提示，
  // 不再像 V2.0.4 那樣完全沒有任何畫面反應。
  if(sigState.autoArmed && signatureStageReady() && isLandscapeViewport() && !sigState.suppressAutoOpen){
    scheduleAutoSignatureOpen();
  }
}
function setupSignature(){
  ...
  const pos=e=>{
    // 手動入口在手機直立時使用 virtual-landscape。
    // 將螢幕座標反算回未旋轉的橫式 wrapper / canvas 座標。
    if(wrap.classList.contains('virtual-landscape')){
      const W=Math.max(1,wrap.offsetWidth),H=Math.max(1,wrap.offsetHeight);
      const cx=window.innerWidth/2,cy=window.innerHeight/2;
      const localWrapX=W/2+(e.clientY-cy);
      const localWrapY=H/2-(e.clientX-cx);
      const left=c.offsetLeft||0,top=c.offsetTop||0;
      const cw=Math.max(1,c.offsetWidth),ch=Math.max(1,c.offsetHeight);
      return {
        x:(localWrapX-left)*(c.width/cw),
        y:(localWrapY-top)*(c.height/ch)
      };
    }
  ...
  const start=e=>{
    if(!wrap.classList.contains('fullscreen'))return;
    if(!currentSignatureAllowed()){e.preventDefault();updateSignatureOrientationGuard();return}
  ...
  const move=e=>{
    ...
    if(!currentSignatureAllowed()){stop(e);return}
    e.preventDefault();e.stopPropagation();
  ...
  // 手動模式只有右上按鈕入口，不會再要求使用者開啟自動旋轉。
  $('toggleSignatureFullscreen').onclick=e=>{e?.stopPropagation?.();enterSignatureFullscreen('manual')};
  ...
  // Auto 模式採「曾經到達簽名區就永久 armed」，
  // 避免手機一轉橫後 IntersectionObserver 因版面重排瞬間變 false，
  // 導致簽名滿版永遠不開。
  if('IntersectionObserver' in window){
    const io=new IntersectionObserver(entries=>{
      if(entries.some(x=>x.isIntersecting&&x.intersectionRatio>0)){
        sigState.inView=true;
        sigState.autoArmed=true;
        maybeAutoSignatureOpen();
      }
    },{threshold:[0,.01,.1,.25]});
    io.observe(wrap);
  }else{
  ...
  const sync=()=>{
    ...
    if(wrap.classList.contains('fullscreen')){
      if(wrap.classList.contains('auto-entry')&&!autoLandscapeState().allowed&&drawing){
        drawing=false;
        last=null;
        setSignatureDrawingLock(false);
      }
      updateSignatureOrientationGuard();
      return;
    }

    // 回到直立後解除前一次完成簽名造成的 Auto 抑制，
    // 下一次重新橫向時仍可正常啟動。
    if(!isLandscapeViewport()&&sigState.suppressAutoOpen&&!signatureDrawn){
      sigState.suppressAutoOpen=false;
    }
  ...
  window.addEventListener('orientationchange',()=>{
    // iPhone / Safari orientationchange 先發生、viewport 後穩定，所以做多段同步。
    setTimeout(sync,80);
    setTimeout(sync,220);
    setTimeout(sync,480);
  });
  ...
  if(window.visualViewport){
    window.visualViewport.addEventListener('resize',()=>setTimeout(sync,80));
  }

  // 前置資料最後完成時也再檢查一次，避免「先橫向、後完成照片」時漏觸發。
...
function enterSignatureFullscreen(mode='manual'){
  ...
  wrap.classList.remove('manual-entry','auto-entry','virtual-landscape','orientation-invalid');
  wrap.classList.add('fullscreen',mode==='auto'?'auto-entry':'manual-entry');

  // 手動模式：若手機仍直立，直接用網站自己的橫向 viewport。
  // 不呼叫 screen.orientation.lock，也不要求使用者再開啟自動旋轉。
  if(mode==='manual'&&window.innerWidth<=window.innerHeight){
    wrap.classList.add('virtual-landscape');
  }
...
function completeSignatureFullscreen(){
  ...
  if(!currentSignatureAllowed()||!sigState.hasInk)return;
...
function exitSignatureFullscreen(){
  ...
  wrap.classList.remove('fullscreen','manual-entry','auto-entry','virtual-landscape','orientation-invalid');
...
function clearSignature(){
  ...
  signatureDrawn=false;sigState.hasInk=false;sigState.normalizedCanvas=null;sigState.suppressAutoOpen=false;
  updateSignatureOrientationGuard();
}
function sigData(){return (sigState.normalizedCanvas||$('sig')).toDataURL('image/png')}
function complete(){return sides.every(([k])=>photos[k]?.uploaded)&&idPhotos.front&&idPhotos.back&&!!customerIdNo&&$('agree').checked&&signatureDrawn}
async function makePreview(silent){if(!complete()){if(!silent)alert('請先完成 6 面照片、證件正反面、條款同意與電子簽名');return null}setProgress(20,'正在產生正式 P1＋P2…');const pages=await R.renderBorrowPages({loan,idNo:customerIdNo,idFront:idPhotos.front,idBack:idPhotos.back,signature:sigData(),borrowPhotos:Object.fromEntries(sides.map(([k])=>[k,photos[k].url])),borrowNote:$('borrowNote').value.trim(),signedAt:new Date().toLocaleString('zh-TW',{hour12:false}),logoUrl:'assets/company-logo.png',p1Target:cfg.pdfPolicy.p1TargetBytes,p2Target:cfg.pdfPolicy.p2TargetBytes});$('p1Prev').src=pages.page1;$('p2Prev').src=pages.page2;$('preview').classList.remove('hidden');setProgress(45,'P1＋P2 已產生，可進行最後確認');return pages}
